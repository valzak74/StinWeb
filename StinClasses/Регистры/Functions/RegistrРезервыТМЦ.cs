...
{
    public class RegistrРезервыТМЦ : IRegistrРезервыТМЦ
    {
        StinDbContext _context;
        public RegistrРезервыТМЦ(StinDbContext context)
        {
            _context = context;
        }
        public async Task<IEnumerable<РегистрРезервыТМЦ>> ПолучитьОстаткиAsync(DateTime dateReg, string idDocDeadLine, bool IncludeDeadLine, List<string> фирмаIds, string договорId, List<string> номенклатураIds, List<string> складIds = null, string заявкаId = null)
        {
        ...
        public async Task<IEnumerable<РегистрРезервыТМЦ>> ПолучитьОстаткиПоЗаявкамAsync(DateTime dateReg, string idDocDeadLine, bool IncludeDeadLine, List<string> фирмаIds, string договорId, List<string> номенклатураIds, List<string> заявкаIds, List<string> складIds = null)
        {
            if (dateReg <= Common.min1cDate)
                dateReg = DateTime.Now;
            if (dateReg >= _context.GetDateTA())
        ...
        public async Task<IDictionary<string, decimal>> ПолучитьКоличествоНоменклатурыВРезервахAsync(DateTime dateReg, string idDocDeadLine, bool IncludeDeadLine, IEnumerable<string> номенклатураIds, string marketplaceId = "")
        {
            if (dateReg <= Common.min1cDate)
                dateReg = DateTime.Now;
            if (dateReg >= _context.GetDateTA())
