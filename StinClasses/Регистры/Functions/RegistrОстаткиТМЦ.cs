...
{
    public class RegistrОстаткиТМЦ : IRegistrОстаткиТМЦ
    {
        StinDbContext _context;
        public RegistrОстаткиТМЦ(StinDbContext context)
        {
            _context = context;
        }
        public async Task<IEnumerable<РегистрОстаткиТМЦ>> ПолучитьОстаткиПоСпискуСкладовAsync(DateTime dateReg, string idDocDeadLine, bool IncludeDeadLine, List<string> фирмаIds, List<string> номенклатураIds, List<string> складIds)
        {
        ...
        public async Task<IEnumerable<РегистрОстаткиТМЦ>> ПолучитьОстаткиAsync(DateTime dateReg, string idDocDeadLine, bool IncludeDeadLine, List<string> фирмаIds, List<string> номенклатураIds, string складId = null, string подСкладId = null)
        {
            if (dateReg <= Common.min1cDate)
                dateReg = DateTime.Now;
            if (dateReg >= _context.GetDateTA())
