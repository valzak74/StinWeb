            ...
            foreach (var фирма in списокАктивныхФирм)
                foreach (var склад in списокАктивныхСкладов)
                {
                    ...
                    doc.ТабличнаяЧастьРазвернутая = активныеНаборы.Where(x => x.Склад.Id == doc.Склад.Id)
                        .SelectMany(x =>
                        x.ТабличнаяЧасть.GroupBy(y => new { НоменклатураId = y.Номенклатура.Id, ЕдиницаId = y.Единица.Id })
                        .Select(gr => new
                        {
                        ...
                        .Join(предварительнаяЗаявка.ТабличнаяЧасть,
                            наборТЧ => new { ннId = наборТЧ.НоменклатураId, неId = наборТЧ.ЕдиницаId },
                        ...
                        .Select(s => new 
                        {
                            Номенклатура = s.ПредЗаявкаТЧ.Номенклатура,
                            Единица = s.ПредЗаявкаТЧ.Единица,
