    ...
    class MarketplaceService : IMarketplaceService
    {
        ...
        public async Task ParseNextPageCatalogAliExpress(string proxyHost, string authToken, string lastProductId, int limit, string marketplaceId, EncodeVersion encoding, CancellationToken stoppingToken)
        {
            ...
            if ((result.Item1 != null) && (result.Item1.Count > 0))
                try
                {
                    ...
                    var responselastProductId = result.Item1.LastOrDefault()?.Id;
                    if (!string.IsNullOrEmpty(responselastProductId) && lastProductId != responselastProductId)
                    {
                        await ParseNextPageCatalogAliExpress(proxyHost, authToken, responselastProductId, limit, marketplaceId, encoding, stoppingToken);
                    }
                }
                catch (Exception ex)
                {
        ...
        public async Task CheckCatalog(CancellationToken stoppingToken)
        {
            ...
            try
            {
                ...
                foreach (var marketplace in marketplaceIds)
                {
                    _logger.LogWarning("Start checking marketplace {marketplaceName} code {marketplaceCode}", marketplace.Наименование, marketplace.CampaignId);
                    ...
                    _logger.LogWarning("Finish checking marketplace {marketplaceName} code {marketplaceCode}", marketplace.Наименование, marketplace.CampaignId);
                }
            }
            catch (Exception ex)
            {
            ...
            _logger.LogWarning("Finish checking all catalogs");
        }
                    ...
                    if ((posting.Products != null) && (posting.Products.Count > 0))
                    {
                        ...
                        if (needCreateDoc)
                        {
                            if (!TimeSpan.TryParse("16:00", out TimeSpan limitTime))
                                limitTime = TimeSpan.MinValue;
                            var leavingDate = DateTime.Now.TimeOfDay > limitTime ? 1 : 0;
                            var firstProductCode = posting.Products.Select(x => x.Offer_id.Decode(encoding)).FirstOrDefault();
                            var shipmentDate = await _marketplace.IsRFbs(id, firstProductCode)
