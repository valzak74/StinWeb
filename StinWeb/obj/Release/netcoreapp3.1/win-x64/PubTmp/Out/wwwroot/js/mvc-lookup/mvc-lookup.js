...
class MvcLookupDialog {
    ...
    open() {
        ...
        if ($(".modal").hasClass("show")) {
            dialog.modalId = $("div.modal.show").attr("id");
            $("#" + dialog.modalId).removeAttr("tabindex");
        }
    ...
    closeWithoutSave() {
        ...
        if (dialog.modalId != undefined || dialog.modalId != "") {
            $("#" + dialog.modalId).attr("tabindex", -1);
            dialog.modalId = "";
        }
...
class MvcLookup {
    constructor(element, options = {}) {
        ...
        lookup.set(options).reload(true);
        lookup.cleanUp();
        lookup.bind();
    ...
    setReadonly(readonly) {
        ...
        if (readonly) {
            ...
            if (lookup.autocomplete) {
                lookup.autocomplete.tabIndex = -1;
                lookup.autocomplete.disabled = true;
            }
            if (lookup.browser) {
            ...
            if (lookup.autocomplete) {
                lookup.autocomplete.disabled = false;
            }
            if (lookup.browser) {
